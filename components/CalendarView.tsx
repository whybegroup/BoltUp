...
export function CalendarView({ events, onSelectEvent }: CalendarViewProps) {
  ...
  const eventsByDate = useMemo(() => {
    ...
    for (const ev of events) {
      const d = ev.start instanceof Date ? ev.start : new Date(ev.start as unknown as string);
      ...
      <View style={styles.eventsSection}>
        ...
        {selectedEvents.length === 0 ? (
          ...
          <View style={styles.eventList}>
            {selectedEvents
              .sort((a, b) => {
                const sa = a.start instanceof Date ? a.start : new Date(a.start as unknown as string);
                const sb = b.start instanceof Date ? b.start : new Date(b.start as unknown as string);
                return sa.getTime() - sb.getTime();
              })
              .map(ev => {
                const group = GROUPS.find(g => g.id === ev.groupId);
                const p = paletteOf(group);
                const startDate = ev.start instanceof Date ? ev.start : new Date(ev.start as unknown as string);
                return (
                  <TouchableOpacity
                    key={ev.id}
