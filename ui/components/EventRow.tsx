...
import { GROUPS, ME_ID, getUser } from '../data/mock';
...
export function EventRow({ ev, onPress, onGroupPress, isLast, showGroup = true }: EventRowProps) {
  ...
  const needsMore = (ev.minAttendees || 0) > 0 && going.length < ev.minAttendees! && !isPast;
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        ...
        {ev.minAttendees! > 0 && !isPast && (
          <Text style={styles.minAttendees} numberOfLines={1}>
            👥 Min {ev.minAttendees} needed{ev.deadline ? ` · RSVP by ${fmtTime(ev.deadline)}` : ''}
          </Text>
        )}
        {going.length > 0 && (
          <View style={styles.avatarRow}>
            <AvatarStack names={going.map(r => getUser(r.userId).displayName)} size={20} max={10} />
          </View>
        )}
        {needsMore && (
          <View style={styles.needsTextWrap}>
            <Text style={styles.needsText}>
              ⚠️ Need {ev.minAttendees! - going.length} more to confirm
            </Text>
          </View>
        )}
        {showHoursLeft && (
