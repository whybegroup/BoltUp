...
interface AvatarSeedPickerProps {
  ...
  /** When provided, shows URL input. GroupAvatar uses thumbnail if valid URL, otherwise seed. */
...
export function AvatarSeedPicker({
  ...
  return (
    <View>
      ...
      <View style={{ marginBottom: 12 }}>
        <Text style={{ fontSize: 12, fontFamily: Fonts.semiBold, color: Colors.textMuted, marginBottom: 6 }}>Choose icon</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingVertical: 4 }}>
          {ICON_OPTIONS.map((icon) => (
            <TouchableOpacity
              key={icon}
              onPress={() => onChangeText(icon)}
              style={{
                width: 44,
                height: 44,
                borderRadius: groupAvatarBorderRadius(44),
                borderWidth: 2,
                borderColor: effectiveSeed === icon ? Colors.accent : Colors.border,
              ...
              activeOpacity={0.8}
            >
              <IconsAvatar seed={icon} size={36} style={{ width: 36, height: 36 }} />
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
      <View style={{ marginBottom: 12 }}>
        <Text style={{ fontSize: 12, fontFamily: Fonts.semiBold, color: Colors.textMuted, marginBottom: 6 }}>Avatar seed (or type custom)</Text>
